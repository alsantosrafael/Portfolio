...
for(let card of cards) {
    ...
    card.addEventListener("click", function() {
        var gitId = card.getAttribute("id")//pegando o atributo id do card para usar como endereco do gif do meu README no github
        modalOverlay.classList.add('active')//adicionando classe active para o modal poder ser aberto
        modalOverlay.querySelector("iframe").src = `${gitId}`//iframe//to adicionando o endereco do video de forma dinamica
                                                    //Vou utilizar esse iframe para passar meu gif do readme direto do github 
   })
